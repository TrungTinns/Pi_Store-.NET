...
namespace PiStore
{
    public partial class ManageClient : Form
    {
        ...
        private void btn_search_Click(object sender, EventArgs e)
        {
            SqlConnection conn = DBConnect.GetInstance();
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM Client WHERE ClientID = @ClientID", conn);
            cmd.Parameters.AddWithValue("ClientID", int.Parse(txt_search.Text));
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            dataGridView1.DataSource = dt;
        ...
        private void btn_searchname_Click(object sender, EventArgs e)
        {
            SqlConnection conn = DBConnect.GetInstance();
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT * FROM Client WHERE ClientName = @ClientName", conn);
